...
if time == True:                     #------------------------------------------------------------------------------------------------------------------------------------------
    ...
    from Basic_Research_Model_for_test_2 import *
    ...
    def finding_Nash(strategy_array, output_matrix):
        ...
        output_matrix   = output_matrix.reshape((2, 2, 2))
        score           = 0
    ...
    for i in range(optimal_trials):
        ...
        Machine                       = Basic_Research_Model(dims, number_of_layers, alpha, initial_epochs, beta, rounds, signal_receiver, signal_transfer, randomness, function, training_method, stachostic)
        ...
        for j in range(total_score_list.shape[0]):
            ...
            if (function == "ReLu") | (function == "Leaky ReLu"):
                random_input_inner_for_player_A   = (np.random.random(strategy_neurons_size) - 0.5 ) * 0.1 + 0.5
                random_input_inner_for_player_B   = (np.random.random(strategy_neurons_size) - 0.5 ) * 0.1 + 0.5



            random_input_inner_for_player_A, random_input_inner_for_player_B = Machine.deduct_from(random_input_inner_for_player_A, random_input_inner_for_player_B, size_of_pain_neurons)
    ...
    axarr[0].plot(x, y)
    axarr[0].set_ylabel(r'$number \, of \, NEs \, found$', fontsize=8)
    axarr[0].set_xlabel(r'$epochs$', fontsize=8)
    axarr[0].set_title(r'$Rev-NN \, Accuracy \, for \, 2x2 \, game \, table$', fontsize = 8)
    axarr[0].set_ylim([0, optimal_trials])
    ...
    for i in range(x.shape[0]):
        x[i] = initial_epochs + i * interval + interval
    axarr[1].plot(x, y)
    axarr[1].set_ylabel(r'$number \, of \, NEs \, found$', fontsize=8)
    axarr[1].set_xlabel(r'$epochs$', fontsize=8)
    axarr[1].set_title(r'$Rev-NN \, Accuracy \, for \, 4x4 \, game \, table$', fontsize = 8)
    axarr[1].set_ylim([0, optimal_trials])
